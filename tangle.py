...
def plot(a, b, labelsA=None, labelsB=None, sort=True, figsize=(16, 80),
    ...
    if isinstance(a, pd.DataFrame):
        ...
        link1 = sclust.hierarchy.linkage(sdist.squareform(a,checks=False), method='average',metric='precomputed')
...
#        link2 = sclust.hierarchy.linkage(sdist.squareform(b, checks=False), **link_kwargs)
        link2 = sclust.hierarchy.linkage(sdist.squareform(b, checks=False), method='average',metric='precomputed')
        if not labelsB:
...
#    Z1 = sclust.hierarchy.dendrogram(link1, orientation='left', labels=labelsA, **dend_kwargs)
    Z1 = sclust.hierarchy.dendrogram(link1, orientation='left', labels=labelsA, leaf_font_size=8.)
    # Compute and plot right dendrogram.
    ax2 = fig.add_axes([0.7, 0.1, 0.25, 0.8])  # [0.3, 0.71, 0.6, 0.2])
    Z2 = sclust.hierarchy.dendrogram(link2, labels=labelsB, orientation='right', leaf_font_size=8.)
